...
interface ExpiredEventGridProps {
  eventos: Evento[]
  searchQuery?: string
}

type FilterKey = 'todos' | 'semana' | 'mes' | 'ano'
...
export default function ExpiredEventGrid({ eventos, searchQuery }: ExpiredEventGridProps) {
  ...
  const filteredEventos = useMemo(() => {
    ...
    if (activeFilter === 'ano') {
      ...
      const startStr = toLocalDateStr(yearStart)
      const endStr = toLocalDateStr(yearEnd)
      return eventos.filter(e =>
        rangesOverlap(e.data_inicio, getEndDate(e), startStr, endStr)
      )
    }
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          <h2 className="font-serif text-4xl md:text-5xl mb-4">
            {searchQuery ? `Resultados para "${searchQuery}"` : "Eventos Expirados"}
          </h2>
          <p className="text-foreground/80">
            {searchQuery
              ? `${filteredEventos.length} eventos encontrados.`
              : activeFilter !== 'todos'
                ? `Mostrando eventos expirados: ${filters.find(f => f.key === activeFilter)?.label}`
                : "Eventos que já aconteceram ou terminaram."
            }
          </p>
        </div>
      ...
      {paginatedEventos.length > 0 ? (
        ...
        <div className="py-20 flex flex-col items-center text-center glass rounded-3xl border-dashed border-2 p-8">
          ...
          <p className="text-foreground/70 font-medium text-xl">
            {searchQuery
              ? `Nenhum evento expirado encontrado para "${searchQuery}"`
              : "Nenhum evento expirado para o período selecionado."
            }
          </p>
